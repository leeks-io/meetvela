              ...
              {messages.length === 0 && (
                <div className="text-center py-6">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-left">
                    {[
                      { label: "What is a PDA?", icon: "🔑" },
                      { label: "Explain ownership", icon: "🦀" },
                      { label: "Write a counter program", icon: "🪙" },
                      { label: "What are traits?", icon: "⚙️" },
                    ].map((q) => (
                      <button
                        key={q.label}
                        onClick={() => send(q.label)}
                        className="flex items-center gap-1.5 px-2.5 py-2 rounded-lg border border-border/50 bg-muted/30 hover:bg-muted/60 hover:border-glow-purple/30 transition-all text-[11px] text-muted-foreground hover:text-foreground"
                      >
                        <span>{q.icon}</span>
                        {q.label}
                      </button>
