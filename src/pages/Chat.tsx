...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-muted/80 text-muted-foreground hover:text-foreground"
      title="Copy response"
    >
    ...
    <div className="h-screen flex flex-col bg-background font-mono">
          ...
          {isEmpty ? (
            ...
            >
              ...
              <div className="text-muted-foreground text-xs space-y-1">
                ...
                <p className="text-muted-foreground mt-2">Type a command or choose a topic below:</p>
              </div>
              ...
              {messages.map((msg, i) => (
                ...
                >
                  {msg.role === "user" ? (
                    ...
                    <div className="group relative pl-4 border-l-2 border-glow-purple/30 ml-1 mt-1 mb-2">
                      <div className="absolute top-0 right-0">
                        <CopyButton text={msg.content} />
                      </div>
                      <div className="prose prose-sm dark:prose-invert max-w-none text-foreground/90 [&_pre]:bg-muted [&_pre]:rounded-lg [&_pre]:p-3 [&_pre]:text-xs [&_pre]:relative [&_code]:text-glow-green [&_code]:text-xs [&_p]:mb-2 [&_p:last-child]:mb-0 [&_h1]:text-base [&_h2]:text-sm [&_h3]:text-sm [&_li]:text-xs">
      ...
      <div className="border-t border-border bg-muted/20">
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-3">
          <div className="flex items-center gap-2">
            <span className="text-glow-green text-sm shrink-0 select-none">vela@solana:~$</span>
            <input
              ref={inputRef}
              value={input}
